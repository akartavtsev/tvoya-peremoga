...
if (!function_exists('mk_page_title')) {
		function mk_page_title()
		{
				...
				if (is_search()) {
						...
						wp_reset_query();
						$subtitle = $count . ' ' . sprintf(__('Результатов поиска: "%s"', 'mk_framework') , stripslashes(strip_tags(get_search_query())));
...
if (!function_exists('mk_theme_breadcrumbs')):
		function mk_theme_breadcrumbs()
		{
				...
				elseif (is_search()) {
						...
						echo __('Результат поиска &ldquo;', 'mk_framework') . get_search_query() . '&rdquo;';
				}
				elseif (is_tag()) {
	...
	<div class="mk-quick-contact-wrapper">
		...
		<div id="mk-quick-contact">
			<div class="mk-quick-contact-title"><?php
				_e('Ваши контакты', 'mk_framework');?></div>
			<p><?php
				_e('Остались вопросы? Присылайте свои письма и мы с радостью дадим ответ!', 'mk_framework');?></p>
			<form class="mk-contact-form" method="post" novalidate="novalidate">
				<input type="text" placeholder="<?php
				_e('Имя*', 'mk_framework'); ?>" required="required" id="contact_name" name="contact_name" class="text-input" value="" tabindex="<?php
				echo $tabindex_1; ?>" />
				...
				_e('Сообщение*', 'mk_framework'); ?>" required="required" id="contact_content" name="contact_content" class="textarea" tabindex="<?php
				echo $tabindex_3; ?>"></textarea>

				<?php
				if ($captcha_quick_contact == 'true') { ?>
				<input placeholder="<?php
						_e('Введите символы с картинки', 'mk_framework'); ?>" type="text" name="captcha" class="captcha-form text-input full" required="required" autocomplete="off" />
		            <a href="#" class="captcha-change-image"><?php
						_e('Не можете прочитать? Изменить текст.', 'mk_framework'); ?></a>
		            <img src="<?php
				...
				_e('Отправить', 'mk_framework'); ?></span>
