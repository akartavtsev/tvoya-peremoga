...
<?php if ( $comment->comment_approved == '0' ) : ?>
					<span class="unapproved"><?php _e( 'Ваш комментарий ожидает модерации.', 'mk_framework' );?></span>
<?php endif; ?>
				<div class="clearboth"></div>
			</div>
...
if ( have_comments() ) : ?>
	<div class="blog-comment-title"><?php printf( _n( 'Comments', 'Комментариев %1$s', get_comments_number(), 'mk_framework' ),
...
<?php endif;?>
	...
	<?php
		...
		$fields =  array(
			'author'=> '<div class="comment-form-name comment-form-row"><input type="text" name="author" class="text_input" id="author" tabindex="54" placeholder="'.__('Имя (Обязательно для заполнения)', 'mk_framework').'"  /></div>',
			'email' => '<div class="comment-form-email comment-form-row"><input type="text" name="email" class="text_input" id="email" tabindex="56" placeholder="'.__('Email (Обязательно для заполнения)', 'mk_framework').'" /></div>',
			'url' 	=> '<div class="comment-form-website comment-form-row"><input type="text" name="url" class="text_input" id="url" tabindex="57" placeholder="'.__('Ваш возраст', 'mk_framework').'" /></div>',
		);

		//Comment Form Args
        $comments_args = array(
			'fields' => $fields,
			'title_reply'=>'<div class="respond-heading">'.__('Оставить комментарий', 'mk_framework').'</div>',
			'comment_field' => '<div class="comment-textarea"><textarea placeholder="'.__('Оставьте Ваш комментарий', 'mk_framework').'" class="textarea" name="comment" rows="8" id="comment" tabindex="58"></textarea></div>',
			'comment_notes_before' => '',
			'comment_notes_after' => '',
			'label_submit' => __('ОСТАВИТЬ КОММЕНТАРИЙ','mk_framework')
		);
